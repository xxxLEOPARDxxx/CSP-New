    ...
    if(CheckAttribute(loc,"type"))
	{
		...
		switch (loc.type)
		{
			...
			case "jungle": // дуэльное поле, лэндфорт и так далее
				...
				if (Whr_IsDay())
				{
					if (CheckAttribute(loc,"id.label") && loc.id.label == "Incas Temple")  // 1.2.3
					{
						SetMusicAlarm("music_Incas_Temple");
					}
					else
					{
						if (CheckAttribute(loc,"id.label") && loc.id.label == "Village")  // 1.2.3
						{
							SetMusicAlarm("music_PearlVillage");
						}
						else
						{
							SetMusicAlarm("music_jungle");
				...
				{
					SetMusicAlarm("music_nightjungle");
				}
