...
//********************************* Доставка Леграна до Мартиники. Квест №1 **********************************
		...
		break;
 		case "Step_F1_5":
			dialog.text = "Ах ты, мерзав"+ GetSexPhrase("ец","ка") +"! Я сообщу генерал-губернатору, как"+ GetSexPhrase("ой негодяй втерся","ая негодяйка втерлась") +" к нему в доверие!";
...
//********************************* Драка с мужем де Лейва. Квест №3 **********************************
 		case "Step_F3_1":
			dialog.text = "Нет, ну как вам нравится... В чужой дом посреди ночи вламывается "+ GetSexPhrase("мужчина","какая-то посторонняя девка") +", а потом спрашивает: 'А в чем дело?!'\n"+
				          "Не стройте из себя идиот"+ GetSexPhrase("а","ку") +"! Моя жена заперта, я нашел у нее это чертово кольцо д'Ожерона.";
		...
		break;
 		case "Step_F6_4":
            if (GetCompanionQuantity(pchar) == 1)   // Заставляем геймера избавиться от взятых кубиков.
            {
				...
				{
					dialog.text = "Ну что же, ты готов"+ GetSexPhrase("","а") +" к тому, чтобы занять место в нашей эскадре. Добро пожаловать и вперед к Кумане!";
            ...
            {
				dialog.text = "Коллега, но я могу взять в это дело тебя одн"+ GetSexPhrase("ого","у") +", на условиях, что твоя доля - четвертая часть. Больше мне компаньоны не требуются. Поэтому избавься от лишних кораблей, если хочешь участвовать. И побыстрей!";
		...
		break;
 		case "Step_F8_10":
			dialog.text = "Ох, пират всегда останется пиратом! Ха-ха!\n"+
				          "Поверьте, "+ GetSexPhrase("милейший","дорогая") +", я даю вам гораздо больше, чем презренный металл! Я даю вам честь и уважение самого короля! А сейчас не смею вас задерживать, вас ждет генерал-губернатор д'Ожерон. Кажется, у него для вас опять какое-то важное дело... Прощайте.";
