...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            if (pchar.questTemp.Ascold == "SeekInformatorHavanna" || pchar.questTemp.Ascold == "2SeekInformatorHavanna")
            {
        		...
        		link.l1 = "Меня зовут " + GetFullName(pchar) + ". Я прибыл"+ GetSexPhrase("","а") +" сюда по поручению Аскольда.";
        		link.l1.go = "Step_1";
            }
...
//<<<<<<<<<<<<======== Разброс диалогов по персонажам ======================
		...
		break;
 		case "Step_2":
    		...
    		link.l1 = "Есть одна проблема. Некоторое время назад из Гаваны ушел мановар, под завязку груженный предметами индейского культа.";
    		link.l1.go = "Step_3";
		break;
 		case "Step_3":
		...
		break;
 		case "Step_5":
        	dialog.text = "Он ушел к бухте Аматике, что южней Белиза. По пути должен сделать остановку у острова Кайман.";
    		link.l1 = "Вот это да! Странное место... И что, так и ушел один?";
    		link.l1.go = "Step_6";
		break;
 		case "Step_6":
        	dialog.text = "Да, ушел один. Дело в том, что Кайман - место сбора и других кораблей с испанских колоний в Америке. Представляешь, в Аматике, дальше вглубь Мейна нашли заброшенный индейский храм, и сейчас там работает королевская экcпедиция. А уж после того, как экспедиция закончит работу, эскадра должна идти в Старый Свет.\n"+
                          "Это все, что я знаю.";
    		link.l1 = "Этого хватает, спасибо, друг. Передам от тебя привет Аскольду.";
    		link.l1.go = "Step_7";
		break;
 		case "Step_7":
        	dialog.text = "Эй, "+ GetSexPhrase("друг","подруга") +", подожди! А деньги?";
