...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				link.l1 = "Джон, я слышал"+ GetSexPhrase("","а") +" о ловцах жемчуга, что промышляют в наших водах. Ты не в курсе этого вопроса?";
        ...
        case "I_know_you_good":
			dialog.text = LinkRandPhrase(GetFullName(pchar) + ", рад тебя видеть! Зачем пожаловал"+ GetSexPhrase("","а") +" на этот раз?",
                                         RandSwear() + " Кого я вижу?! Да это мой друг " + GetFullName(pchar) + ". Какими судьбами на острове?",
                                         "Так-так, "+ GetFullName(pchar)+ " у меня в гостях. Хотел бы я еще раз увидеть, как ты управляешься с клинком... Ну что, какие вопросы, "+ GetSexPhrase("друг","подруга") +"?");
			link.l1 = RandPhraseSimple("Просто заскочил"+ GetSexPhrase("","а") +" проведать тебя.", "Мелкие дела, ничего существенного.");
			link.l1.go = "exit";
		...
		//********************* капитан Шарп *********************
		case "SharpPearl_1":
			dialog.text = "Нет, не в курсе. Это - к Шарпу, "+ GetSexPhrase("приятель","подруга") +". Язык не поворачивается назвать его капитаном...";
 		...
 		case "EngLine_quests":
			...
			switch (pchar.questTemp.State)   // Доступ к квестам
            {
	     		case "go_talks_with_pirates":
	    			...
	    			link.l1 = "Я приш"+ GetSexPhrase("ел","ла") +" к тебе по поручению генерал-губернатора Ямайки, сэра Томаса Мэдифорда.";
	    			link.l1.go = "Step_1";
	      		break;
            }
		break;
 		...
 		case "Step_1":
			dialog.text = "Уже интересно! Мэдифорд меня не жалует, что же его заставило послать ко мне "+ GetSexPhrase("пирата","своего человека") +"?";
 		...
 		case "Step_4":
			dialog.text = "Плохо ты знаешь Мэдифорда. Нет, я не так глуп, что-то здесь не то. Возможно, предстоит кровавая баня, а мне как-то не хочется умываться кровью ради генерал-губернатора Ямайки. Так что без меня, и удачи тебе, "+ GetSexPhrase("корсар","подруга") +".";
		...
		break;
        case "Step_F8_Result":
            dialog.text = "Ну, как успехи, капитан? Раздобыл"+ GetSexPhrase("","а") +" судовой журнал этого мерзавца Гея?";
