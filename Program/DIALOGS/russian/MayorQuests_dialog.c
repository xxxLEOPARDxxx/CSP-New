void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "SeekSpy_1":
			dialog.text = "Ну, а сам"+ GetSexPhrase("","а") +"-то ты кто "+ GetSexPhrase("такой","такая") +", чтобы я отвечал на твои вопросы?";
		...
		case "SeekSpy_street":
			...
			link.l5 = PCharRepPhrase("Хотел"+ GetSexPhrase("","а") +" кое-что узнать об этом городишке!", "Хотел"+ GetSexPhrase("","а") +" узнать кое-что о городе.");
	  ...
	  // to_do
		...
		case "DP_toShore":
			...
			Link.l1 = "Тогда я готов"+ GetSexPhrase("","а") +". За дело.";
			Link.l1.go = "exit";
		...
		case "DP_FightInDeck":
			dialog.text = "Надо же, "+ GetSexPhrase("какой храбрец","какая отважная девка") +"! Ценю смелость, жаль будет тебя убивать...\nЭй, на палубе, валите "+ GetSexPhrase("этого","эту") +" выскочку аккуратно, я пока подготовлю корабль к бою. Только без лишнего шума!";
