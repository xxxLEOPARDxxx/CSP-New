...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	switch (Dialog.CurrentNode)
	{
		...
		//============================== Квест Аззи ===============================
 		case "Step_Az6":
        	dialog.text = "О-о-о, Cэр Аззи прислал тебя ко мне?! Очень рад слышать о нем - такой приятный и деловой человек! И что же он тебе сказал?";
    		link.l1 = "Человек?! Хм, ну ладно... Он сказал мне, что я могу брать у тебя ром бесплатно и сколько захочу.";
    		link.l1.go = "Step_Az7";
		break;

 		case "Step_Az7":
        	dialog.text = "Ну, не совсем так. Ты можешь брать у меня 300 ц рома один раз в день. И, разумеется, бесплатно.";
		...
		break;
 		...
 		case "Step_Az8":
        	dialog.text = NPCStringReactionRepeat("Я могу дать тебе 300ц. рома.",
                       "Ты уже "+ GetSexPhrase("брал","брала") +" сегодня положенный тебе ром.",
                       "Я же говорю тебе, что свой ром на сегодня ты "+ GetSexPhrase("выбрал","выбрала") +". Приходи завтра.",
                       "Завтра, приходи завтра...", "block", 1, npchar, Dialog.CurrentNode);
    		link.l1 = HeroStringReactionRepeat("Хорошо, беру все!!!", "Верно, "+ GetSexPhrase("брал","брала") +"...", "Извини...", "Ну, тогда до завтра.", npchar, Dialog.CurrentNode);
		...
		break;
 		...
 		case "Step_Az12":
        	dialog.text = "А вот этого я бы делать не стал. Мой помощник полюбопытствовал уже, спрятался днем в сарае, подсмотреть хотел. Только утром нашли его рядом с ромом тронутым, теперь сидит целыми днями, слюни пускает и мычит.\n"+
                          ...
                          "А увидеть ты все одно ничего не увидишь. Запирал я уже сарай, обойдусь, думаю, без рома. А ром снова утром на месте. Вот так, "+ GetSexPhrase("дружище","подруга") +".";
