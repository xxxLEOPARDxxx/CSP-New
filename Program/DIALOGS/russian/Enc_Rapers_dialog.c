...
//			Diag.TempNode = "OnceAgain";
		...
		case "Node_3":
			...
			dialog.text = "Да она обошлась нам в кругленькую сумму! Если ты так"+ GetSexPhrase("ой","ая") +" благородн"+ GetSexPhrase("ый","ая") +", то можешь забирать её за " + sti(pchar.GenQuest.EncGirl.price) + " пиастров."+ GetSexPhrase(" Покувыркаешься вволю, хе-хе..","") +"";
			...
			link.l2 = LinkRandPhrase("Пиастров захотелось?! А сталь под ребро не желаешь?!",
		...
		case "Node_7":
			...
			switch (i)
			{
				...
				break;
				case 2:
					pchar.GenQuest.EncGirl.FatherNom = "служит комендантом форта";
					pchar.GenQuest.EncGirl.FatherGen = "коменданта форта";
		...
		case "Node_9":
			...
			dialog.text = "Что твоя сумма по сравнению с сокровищами " + GetName(pchar.GenQuest.EncGirl.PirateIdx , pchar.GenQuest.EncGirl.PirateName1, NAME_GEN) + "? " +
							...
							"Жалкую горсть пиастров?! Всю добычу " + GetName(pchar.GenQuest.EncGirl.PirateIdx , pchar.GenQuest.EncGirl.PirateName1, NAME_NOM) + " утаскивал к себе в тайник. Но теперь мы не намерены отступать! Показывай, где его сундуки!";
		...
		case "EncGirl_Berglar2":
			...
			{
				dialog.text = "Нет. Скромная сумма в " + sti(pchar.GenQuest.EncGirl.BerglarSum) + " пиастров может избавить тебя от любых неприятностей.";
