...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First":
			...
			link.l1.go = "Meeting";
			if (pchar.sex != "woman") LAi_CharacterPlaySound(NPChar, "Salazar_1_man");
			else LAi_CharacterPlaySound(NPChar, "Salazar_1_woman");
		break;
		...
		case "Meeting":
			...
			LAi_CharacterPlaySound(NPChar, "Salazar_2");
		break;
		...
		case "Meeting_2":
			...
			link.l1.go = "exit";
			LAi_CharacterPlaySound(NPChar, "Salazar_3");
		break;
		...
		case "Last":
			dialog.text = "Ты меня впечатлил"+ GetSexPhrase("!","ла!") +" Никто из тех, кто прежде отваживался прийти сюда, не могли одолеть нас и оставались здесь навсегда.";
			link.l1 = "Это было очень тяжело. Ты и твои ребята заставили меня облиться семью потами. Да и чувствую я себя неважно.";
			link.l1.go = "Last_2";
			if (pchar.sex != "woman") LAi_CharacterPlaySound(NPChar, "Salazar_4_man");
			else LAi_CharacterPlaySound(NPChar, "Salazar_4_woman");
		break;
		...
		case "Last_2":
			...
			link.l1.go = "Last_3";
			LAi_CharacterPlaySound(NPChar, "Salazar_5");
		break;
		...
		case "Last_3":
			...
			LAi_CharacterPlaySound(NPChar, "Salazar_6");
		break;
