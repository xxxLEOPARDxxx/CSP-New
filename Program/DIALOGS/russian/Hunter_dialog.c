void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "meeting":
            ...
            {
                Link.l4 = "Хорошо, я проследую с вами. Я сам"+ GetSexPhrase("","а") +" уже "+ GetSexPhrase("собирался","собиралась") +" все уладить.";
    			Link.l4.go = "ToPrison";
            }*/
		break;
		...
		case "ReasonToFast_THunter_2":
			...
			dialog.text = "Э нет, " + GetSexPhrase("господин хороший","милочка") + ". Так дело не пойдёт... Мы точно знаем, что карта " + GetName( pchar.questTemp.ReasonToFast.mapIdx, pchar.questTemp.ReasonToFast.map, NAME_GEN) +" досталась тебе. А он был изрядным скрягой, и его сундуки ломились от пиастров. Поэтому с пустыми руками мы не уйдём.";
		...
		case "ReasonToFast_HunterShore21":
			dialog.text = "Не по чину... Ты сорвал"+ GetSexPhrase("","а") +" операцию, из-за чего мы лишились " + sti(pchar.questTemp.ReasonToFast.p10) + " пиастров. Так что с тебя должок.";
			link.l1 = "Забирайте и передайте " + GetName( NAMETYPE_MAIN, pchar.questTemp.ReasonToFast.p3, NAME_DAT) + " привет. Скажите, что скоро встретимся...";
