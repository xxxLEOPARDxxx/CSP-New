...
void SetVariable()
{
	...
	sText = sText; // {*} BUHO: ?????? - What is ??? doing? //Boyer: Bit of a hack: for some reason sText loses its 'string' type without an assignment
	//Boyer add from GOF
	//// {*} BUHO-PT - ADDED CODE - Ship cannons in cannon shop.
	// Show ship cannon state in cannon shop window.
	{
	// Entry sText: "1530 / 2700" (cargo hold)
	int iCannonIx = sti(refCharacter.Ship.Cannons.Type);
	int iCannonType = sti(Cannon[iCannonIx].Type);

	sText = sText + "\n";
	switch (iCannonType)
		{
		case CANNON_NAME_CANNON			: sText = sText + "Пушки "; break;
		case CANNON_NAME_CULVERINE		: sText = sText + "Кулев. "; break;
		case CANNON_NAME_SPECIAL_CANNON	: sText = sText + "Спец "; break;
		}
	sText = sText + GetCannonCaliber(sti(refCharacter.Ship.Cannons.Type)) + "фт";
	sText = sText + "\n";
	sText = sText + GetIntactCannonQuantity(refCharacter) + " / " + GetCannonQuantity(refCharacter);
	// Exit sText: "1530 / 2700 \n CNN 20lbs \n 28 / 32" (cargo hold + ship cannon state)
	}
	//// {*} BUHO END ADDITION
