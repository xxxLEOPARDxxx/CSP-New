...
void InitInterface_R(string iniName, ref pStore)
{
	...
	GameInterface.TABLE_LIST.hr.td8.scale = 0.9; */ // LEO: Забекапил, чтобы не обосраться в очередной раз :)
	...
	if(refStore.Colony == "none")
	{
		if(CheckAttribute(refStore, "StoreHouse"))
		{
			SetFormatedText("STORECAPTION1", "Склад");
		}
		else
		{
 	...
 	if(refStore.Colony == "none")
	{
	 	if(CheckAttribute(refStore, "StoreHouse"))
		{
			SetNewGroupPicture("OTHER_PICTURE", "COLONY_BUILDINGS_1", "StoreHouse");
		}
		else
		{
...
void AddToTable()
{
    	...
    	for(i = 0; i< GOODS_QUANTITY; i++)
	{
		...
		GameInterface.TABLE_LIST.(row).td5.str = ""; */ // LEO: Забекапил, чтобы не обосраться в очередной раз :)
...
void SetVariable()
{
	...
	if(refStore.Colony == "none")
	{
		if(CheckAttribute(refStore, "StoreHouse"))
		{
			iTotalSpace = sti(refStore.max_weight);
			sMaxGoodsStore = "Общая загруженность склада" + ": " + makeint(fStoreWeight) + " / " + iTotalSpace;
		}
		else
		{
...
void SetShipWeight()
{
    	...
    	if(refStore.Colony == "none")
	{
		if(CheckAttribute(refStore, "StoreHouse"))
		{
	    		fStoreWeight = makeint(refStore.current_weight);
		}
		else
		{
...
void TransactionOK()
{
 	...
 	if (BuyOrSell == 1) // BUY
	{
        if(refStore.Colony != "none")
		{
   			SetStoreGoodsEdition(refStore, iCurGoodsIdx, iStoreQty - nTradeQuantity, nTradeQuantity, true);
		}
		else
		{
			if(CheckAttribute(refStore, "StoreHouse"))
	...
	{ // SELL
        	if(refStore.Colony != "none")
		{
      			SetStoreGoodsEdition(refStore, iCurGoodsIdx, iStoreQty + nTradeQuantity, nTradeQuantity, false);
		}
		else
		{			
			if(CheckAttribute(refStore, "StoreHouse"))
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
		    ...
		    // проверка на колво доступное <--
		    SetFormatedText("QTY_TypeOperation", "Перегрузить на склад");
		...
		else
		{
			...
			SetFormatedText("QTY_TypeOperation", "Перегрузить со склада");
