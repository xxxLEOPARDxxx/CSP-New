...
void InitInterface_R(string iniName, ref _char)
{
    ...
    if (bChangePIRATES)
    {
    	ShowMsgMenu(); // инфо чЄ делать в начале
    }
}
...
void IDoExit(int exitCode)
{
	if (bChangePIRATES)
	{
		...
		if (Pchar.questTemp.CapBloodLine != true && Pchar.questTemp.WhisperLine != true)//09/08/07 homo дл€ Ѕлада даем другое начало
		{
...
void FillSkillTables()
{
	...
	GameInterface.TABLE_OTHER.tr10.td2.str = "–егенераци€ жизни";
	...
	GameInterface.TABLE_OTHER.tr11.td2.str = "ќтравление";
	...
	GameInterface.TABLE_OTHER.tr12.td2.str = " ровотечение";
	...
	GameInterface.TABLE_OTHER.tr13.td2.str = "–езкий удар";
	...
	GameInterface.TABLE_OTHER.tr14.td2.str = "Ўанс промаха ќ/ѕ";
	...
	GameInterface.TABLE_OTHER.tr15.td2.str = "Ўанс/урон крита";
	...
	GameInterface.TABLE_OTHER.tr16.td2.str = "¬рем€ перезар€дки";
...
//методы расчЄта показателей в таблице - Gregg
string ShowStatValue(string type)
{
	switch (type)
	{
		...
		case "misschanceheavy":
			...
			if(CheckCharacterPerk(xi_refCharacter, "Energaiser")) // скрытый перк боссов и √√
			{
...
void SetSkillArrows()
{
	...
	if (sti(xi_refCharacter.skill.freeskill) > 0 && ok && !bChangePIRATES)  //дл€ начала игры фиг
	{
	...
	ok = sti(xi_refCharacter.skill.freeskill) > 0 || sti(xi_refCharacter.skill.FreeSPECIAL) > 0 || bChangePIRATES;  //дл€ начала игры
...
void FillControlsList(int nMode)
{
	switch (nMode)
	{
	    case 1: FillPerksTable("Self", true); break;  // Ћичные перки
	    case 2: FillPerksTable("Ship", true); break;  //  орабельные перки
	}
}
...
void IncreaseSkill(int _add)
{
    ...
    else
    {
	    ...
	    if (bChangePIRATES)
	    { //дл€ начала игры пересчет скилов
...
void ShowMsgMenu()
{
	SetFormatedText("MSG_WINDOW_CAPTION", "—оздать своего геро€");
	SetFormatedText("MSG_WINDOW_TEXT", "ƒл€ изменени€ параметров геро€ укажите характеристику и стрелками перераспределите очки. ”мени€ автоматически пересчитаютс€ от выбранных PIRATES. «акройте форму крестиком или Esc, когда распределение будет завершено.");
