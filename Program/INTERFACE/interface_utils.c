...
void SetShipQualityTable(ref _chr, string _bar1, string _bar7, string _bar2, string _bar3, string _bar4, string _bar5, string _bar6)
{
	...
	if (sti(refShip.BaseType) <= SHIP_SP_SANFELIPE)//рассчитываем качество только дл€ Ќ≈квестовых кораблей
	{
	...
	float MaxSpeedBONUS = (stf(rBaseShip.SpeedRate) * MaxiCannonDiff) / (15*iDiffClass);//рассчитываем бонусы от неполного числа пушек при генерации корабл€
...
//=======================================//
	...
	fTemp = stf(refShip.OptCrew); 	//тюнинг не вли€ет на оптимальную
...
// Warship 11.07.09 ¬ывести в текстовое поле инфу о количестве дней, на сколько хватит рому на судне
...
{
	bool bNoSize;
	if (_iLinesCount < 1) {bNoSize = true; _iLinesCount = 998;} else bNoSize = false;//если число строк не передано, ставим 998 строк дл€ цикла
	...
	for (m = 0; m < _iLinesCount-2; m++)
	{
		...
		sCurRow = "tr" + (n + 1);
		sNextRow = "tr" + (n + 2);
		if (bNoSize) {if (!checkattribute(&GameInterface,sControl + "." + sNextRow)) {_iLinesCount = n+2; bNoSize = false; break;}}//если число строк не было указано, провер€ем, не кончилась ли таблица
//находим число строк - проверить, что это оптимально, и что мусорное значение не сломает//TODO - об€зательно разобратьс€!!! ¬ј∆Ќќ!!!

//TODO - убрать это деление на строки, автоматически определ€ть - текст или число сортируем
		if (bIsString)//строки
			{
