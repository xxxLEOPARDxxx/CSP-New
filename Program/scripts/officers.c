		...
		if(cn!=-1)
		{
			...
			if (CheckAttribute(sld, "loyality"))
			{
				...
				else
				{
					...
					{
						sld.loyality = makeint(sld.loyality) + 1;// специально не делаю приведение к 0 и мах. потому чтоб был запас минусов дл€ проверок
...
void OfficersReactionResult()
{
	...
	if (chrDisableReloadToLocation) return; // уже увольн€етс€
...
// метод увольн€ет офицера - название левое, наследие к3
void CheckForReleaseOfficer(int iCharIndex)
{
	...
	//}
	DeleteAttribute(&characters[iCharIndex], "isfree"); // дл€ формы ф2, название странное - обратный смысл, если есть, то зан€т
}
...
void SetOfficerParam(ref Npchar, int _type)
{
	...
	ClearCharacterExpRate(Npchar); // трем все пороги экспы на вс€к сулчай
    ...
    switch(_type)
	{
		case 0:
			...
			Npchar.quest.officertype_2 = RandPhraseSimple("ћогу и доктором побыть, если припрет. ", "≈ще умею матросов беречь, кости им вправл€ть, когда подран€т. ");
		...
		case 2:
			Npchar.quest.officertype_2 = RandPhraseSimple("ћогу и корабль починить помочь. ", "≈ще умею с пилой и рубанком обращатьс€. ");
		...
		case 3:
			Npchar.quest.officertype_2 = RandPhraseSimple("≈ще поверхностно медицину знаю. ", "¬рачем, конечно, не считаюсь, но подскажу и в этом. ");
...
void LandEnc_OfficerFired()
{
    ...
    DeleteAttribute(sld, "Payment"); // признак офицера дл€ +1 предметов  
