...
void PlaceSmugglersOnShore(string LocationId)
{
	...
	for (i = 1; i <= 3+makeint(MOD_SKILL_ENEMY_RATE/2); i++)
    {
		...
		Smuggler = GetCharacter(NPC_GenerateCharacter("Rand_Smug0" + i, Model, "man", "man", 5, PIRATE, 4, true)); // 4 дн€, потом сами пропадут
...
void SetCoastalGuardPursuit()
{
	...
	for (i = 1; i <= 3; i++)
    {
		...
		Smuggler = GetCharacter(NPC_GenerateCharacter("Coastal_Captain0" + i, Model, "man", "man", 5, iNation, 3, true)); // 3 дн€, потом сами пропадут
		SetFantomParam(Smuggler);
