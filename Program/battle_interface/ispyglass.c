...
void SetSpyGlassData()
{
	...
	if (CheckAttribute(arScopeItm,"scope.show.cannontype") && sti(arScopeItm.scope.show.cannontype)== true)
	{
		...
		{
			if (CheckCharacterPerk(chref, "MusketsShoot")) sCaptainName = "  Њушкетный залп   " + sCaptainName;
			if (CheckAttribute(RealShips[sti(chref.Ship.Type)],"Tuning.HighBort")) sCaptainName = "  ‚ысокие борта  " + sCaptainName;
		}
